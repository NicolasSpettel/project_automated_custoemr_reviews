...
@st.cache_data
def load_data():
    """Load the processed data with caching for performance."""
    try:
        df = pd.read_csv("../data/processed/reranker_products.csv")
        return df
    except FileNotFoundError:
        st.error("Data file 'reranker_products.csv' not found. Please ensure the file is in the same directory.")
        return None
...
def classify_product_category(text: str) -> Dict[str, Any]:
    """Classify product category using zero-shot classification."""
    categories = [
    ...
    try:
        ...
        category_keywords = {
            "Electronics & Technology": ["phone", "computer", "tablet", "tv", "camera", "headphones", "electronic"],
            "Home & Garden": ["kitchen", "furniture", "decor", "garden", "home", "cleaning"],
            "Books & Media": ["book", "novel", "story", "read", "author", "kindle"],
            "Health & Beauty": ["health", "beauty", "skincare", "makeup", "supplement", "vitamin"],
            "Sports & Outdoors": ["sport", "fitness", "outdoor", "exercise", "bike", "running"],
            "Clothing & Accessories": ["clothes", "shirt", "dress", "shoes", "fashion", "wear"]
        }
...
def generate_category_summary(df: pd.DataFrame, category: str, summarizer) -> str:
    ...
    # Get sample positive reviews for summarization
    positive_reviews = category_data[category_data['rating'] >= 4]['text'].dropna()
    ...
    # Generate AI summary of positive reviews
    if len(positive_reviews) > 0 and summarizer:
        try:
            sample_reviews = " ".join(positive_reviews.sample(min(5, len(positive_reviews))).tolist())
            if len(sample_reviews) > 100:
                ai_summary = summarizer(sample_reviews[:1000], max_length=100, min_length=30)
                summary_text += f"\n**Customer Highlights:** {ai_summary[0]['summary_text']}"
        except Exception as e:
            summary_text += f"\n**Note:** AI summarization temporarily unavailable."
...
def show_dashboard_overview(df: pd.DataFrame):
    ...
    st.dataframe(category_stats, width=True)
    ...
    if st.button("Analyze Text", type="primary"):
        if user_text.strip():
            ...
            if summarizer and len(user_text) > 50:
                ...
                st.write("Enter longer text (50+ characters) for AI summarization.")
        else:
...
def show_category_analysis(df: pd.DataFrame, summarizer):
    ...
    if selected_category:
        ...
        st.dataframe(top_products, width=True)
...
def show_product_search(df: pd.DataFrame):
    ...
    if search_query:
        ...
        if len(search_results) > 0:
            ...
            st.dataframe(product_summary, width=True)
...
def show_upload_analysis(sentiment_classifier, summarizer):
    ...
    if uploaded_file is not None:
        try:
            ...
            st.dataframe(upload_df.head(), width=True)
    ...
    if sentiment_classifier:
        ...
        for i, text in enumerate(sample_df[text_col].astype(str)):
            if pd.notna(text) and text.strip():
                try:
                    result = sentiment_classifier(text[:500])[0]  # Limit text length
        ...
        st.dataframe(sample_results[[text_col, 'Predicted_Sentiment']].head(10), width=True)
